...
export function connUrl(conn: ResolvedConnection): string {
  switch (conn.collection) {
    case 'posts':     return `/writing/${conn.id}`;
    case 'notes':     return `/notes/${conn.id}`;
    case 'memories':  return `/memory/${conn.id}`;
    case 'fragments': return `/fragments#${conn.id}`;
    case 'excerpts':  return `/excerpts#${conn.id}`;
  }
}
...
async function resolveOne(connStr: string): Promise<ResolvedConnection | null> {
  ...
  try {
    switch (col) {
      case 'posts':     return await getEntry('posts', id) ?? null;
      case 'notes':     return await getEntry('notes', id) ?? null;
      case 'memories':  return await getEntry('memories', id) ?? null;
      case 'fragments': return await getEntry('fragments', id) ?? null;
      case 'excerpts':  return await getEntry('excerpts', id) ?? null;
      default:
...
export async function resolveConnections(connections?: string[]): Promise<ResolvedConnection[]> {
  ...
  return resolved.filter((r): r is ResolvedConnection => r !== null);
}
